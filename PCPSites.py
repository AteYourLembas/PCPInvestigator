...
class HealthGrades(Scrapeable):
	...
	def scrape(self, nameTerms=[]):
		...
		return self._scrape(html, nameTerms)


	######################################
	# _scrape
	######################################	
	def _scrape(self, html, nameTerms=[]) :
...
class UCompareHealthCare(Scrapeable):
	...
	def scrape(self, nameTerms=[]):
		...
		#
		self.sourcePath = "%s/reviews.html"%("http://%s"%"/".join(self.sourcePath.split("/")[2:5]))
	...
	def _scrape(self, html, nameTerms=[]) :
		...
		if nameTerms :
			...
			businessName = " ".join([mainName, suffix])
			if not self.nameMatches(businessName, nameTerms) :
				print "Name does not match satisfactorily."
				return False
...
class Vitals(Scrapeable):
	...
	def _scrape(self, html, nameTerms=[]) :
		...
		if nameTerms :
			...
			# Verify the name is what we expect
			#
			reviewspane = soup.find("div", {"id": "reviewspane"})

			if not reviewspane :
				print "No reviews pane found."
				return False

			businessName = reviewspane.find("span", "fn").string
			if not self.nameMatches(businessName, nameTerms) :
				print "Name does not match satisfactorily."
				return False
